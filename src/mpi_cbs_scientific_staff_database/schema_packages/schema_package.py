...
class ScientificStaffProfile(ElnBaseSection, EntryData):
    ...
    def normalize(self, archive: 'EntryArchive', logger: 'BoundLogger') -> None:
        ...
        if self.expertise_summary:
            self.description = self.expertise_summary

        super().normalize(archive, logger)

        if self.email and archive.results and archive.results.eln:
            if archive.results.eln.names is None:
                archive.results.eln.names = []
            if self.email not in archive.results.eln.names:
                archive.results.eln.names.append(self.email)
